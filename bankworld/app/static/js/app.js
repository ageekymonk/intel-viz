...
(function() {
  ...
  HeatMap = (function() {
    ...
    HeatMap.prototype.load = function(d, c, colselections) {
      var dateFormat;
      if (colselections == null) {
        colselections = ['act1', 'act2'];
      }
      this.alldata = d;
      this.alllocdata = c;
      this.selections = colselections;
      dateFormat = d3.time.format("%Y-%m-%d %H:%M:%S");
    ...
    HeatMap.prototype.draw = function(start_time, end_time, timecount) {
      ...
      yAxis = d3.svg.axis().scale(yScaleHeat).orient('left').ticks(10);
      ...
      for (i = _j = 0, _ref1 = this.colPick.length; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; i = 0 <= _ref1 ? ++_j : --_j) {
        svgHeat.selectAll("rect" + i).data(dataset).enter().append('rect').attr({
          ...
          mouseover: function(d) {
            ...
            return showregion(d.region, d.timestamp);
          },
          mouseout: function(d) {
      ...
      return title = svgHeat.selectAll('.title').data(this.colPick).enter().append('text').text((function(_this) {
        ...
        x: (function(_this) {
          return function(d, i) {
            return i * _this.titleWidth / _this.colPick.length;
          };
        })(this),
        y: this.padding / 2,
        "transform": "translate(" + (this.titleWidth / this.colPick.length) + ", 0)",
